...
class RandomOrganoidPairDataset(Dataset):
    def __init__(self,
        ...
        # Check if data_dir contains subdirectories or files directly
        for item in os.listdir(data_dir):
            item_path = os.path.join(data_dir, item)
            
            if os.path.isdir(item_path):
                # If it's a subdirectory, scan files in the subdirectory
                for filename in os.listdir(item_path):
                    ...
                    if filename.endswith(".tif"):
                        ...
                        self.grouped_examples[str(fileclass)].append(os.path.join(item_path, filename))
            else:
                # If it's a file directly in data_dir
                if item.endswith("_mask.tif"):
                    continue

                if item.endswith(".tif"):
                    identifier = int(os.path.splitext(item)[0].split("_")[-1])
                    fileclass = identifier // 100

                    self.grouped_examples[str(fileclass)].append(os.path.join(data_dir, item))
                ...
                if os.path.isdir(item_path):
                    ...
                    for filename in os.listdir(item_path):
                        ...
                        if filename.endswith(".tif"):
                            ...
                            self.paths.append(os.path.join(item_path, filename))
                            self.labels.append(str(fileclass))
                else:
                    # If it's a file directly in data_dir
                    if item.endswith("_mask.tif"):
                        continue
                    ...
                    if item.endswith(".tif"):
                        ...
                        self.paths.append(os.path.join(data_dir, item))
                        self.labels.append(str(fileclass))
                ...
                if os.path.isdir(item_path):
                    ...
                    for filename in os.listdir(item_path):
                        ...
                        if filename.endswith(".tif"):
                            ...
                            self.paths.append(os.path.join(item_path, filename))
                            self.organoid_classes.append(str(fileclass))
                else:
                    # If it's a file directly in data_dir
                    if item.endswith("_mask.tif"):
                        continue
...
class DeterministicOrganoidHistDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        image = imread(path)
        hist = self.compute_histogram(np.uint16(image[:, :, :3]))
...
class OnlineDeterministicOrganoidHistPairDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        hist_1 = self.compute_histogram(np.uint16(image_1[:, :, :3]))

        image_2 = imread(path_2)
        hist_2 = self.compute_histogram(np.uint16(image_2[:, :, :3]))
